...
namespace VGMToolbox.format.util
{
    ...
    public sealed class GenhUtil
    {
        ...
        public static int BytesToSamples(int formatId, int byteValue, int channels, int interleave)
        {
            ...
            switch (formatId)
            {
                case 0x00: // "0x00 - PlayStation4位ADPCM"
                case 0x0E: // "0x0E - PlayStation4位ADPCM (带错误标志)
                    sampleCount = (byteValue / 16 / channels * 28);
                    break;

                case 0x01: // "0x01 - XBOX 4位IMA ADPCM"
                    sampleCount = (byteValue / 36 / channels * 64);
                    break;

                case 0x02: // "0x02 - 任天堂GameCube ADP/DTK 4位ADPCM"
                    sampleCount = (byteValue / 32 * 28);
                    break;

                case 0x03: // "0x03 - PCM RAW (大端序)"
                case 0x04: // "0x04 - PCM RAW (小端序)
                    sampleCount = (byteValue / 2 / channels);
                    break;

                case 0x05: // "0x05 - PCM RAW (8位)"
                case 0x0D: // "0x0D - PCM RAW (8位无符号)"
                    sampleCount = (byteValue / channels);
                    break;

                case 0x06: // "0x06 - Squareroot-delta-exact 8位DPCM"
                    sampleCount = (byteValue / 1 / channels);
                    break;

                case 0x08: // "0x08 - MPEG音频层文件(MP1/2/3)"
                    // Not Implemented
                    break;

                case 0x07: // "0x07 - Interleaved DVI 4位IMA ADPCM"
                case 0x09: // "0x09 - 4位IMA ADPCM"
                case 0x0A: // "0x0A - Yamaha AICA 4位ADPCM"    
                case 0x11: // "0x11 - 苹果Quicktime 4位IMA ADPCM"    
                    sampleCount = (byteValue / channels * 2);
                    break;

                case 0x0B: // 0x0B - 微软4位ADPCM
                    frames = byteValue / interleave;
                    lastFrame = byteValue - (frames * interleave);
                    sampleCount = (frames * (interleave - (14 - 2))) + (lastFrame - (14 - 2));
                    break;

                case 0x0C: // "0x0C - 任天堂GameCube DSP 4位ADPCM"
                    sampleCount = (byteValue / channels / 8 * 14);
                    break;

                case 0x0F: // "0x0F - 微软4位IMA ADPCM"
