    ...
    public class XsfUtil
    {
        ...
        public static void Time2sfFolder(Time2sfStruct pTime2sfStruct, out string pMessages)
        {
            ...
            if (Sdat.IsSdat(pTime2sfStruct.SdatPath))
            {
                ...
                {
                    Bass.Init(-1, 44100, DeviceInitFlags.Default, IntPtr.Zero, IntPtr.Zero);
                }
            ...
            string p2sfFilePath, string pSseqFilePath, out string pErrorMessage)
        {
            ...
            try
            {
                ...
                {
                    midiStream = BassMidi.CreateStream(midiFilePath, 0L, 0L, BassFlags.Default, 0);

                    if (midiStream != 0)
                    {
                        // play the channel
                        long length = Bass.ChannelGetLength(midiStream);
                        double tlength = Bass.ChannelBytes2Seconds(midiStream, length);
                    ...
                    {
                        // error
                        pErrorMessage = Environment.NewLine + String.Format("Stream error: {0}", Bass.LastError) +
                            Environment.NewLine;
                        return false;
                    }
            ...
            for (int i = pMinIndex; i <= pMaxIndex; i++)
            {
                using (MemoryStream mini2sfMs = new MemoryStream(mini2sfData, true))
                {
                    ...
                    using (MemoryStream compressedMs = new MemoryStream())
                    {
                        ...
                        byte[] crcBuf = compressedMs.ToArray();
                        crc32Calc.Update(new ArraySegment<byte>(crcBuf, 0, crcBuf.Length));
                        mini2sfCrc32 = (int)crc32Calc.Value;
