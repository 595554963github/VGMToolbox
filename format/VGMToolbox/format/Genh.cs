...
namespace VGMToolbox.format
{
    public class Genh : IFormat
    {
        ...
        public const string ATRAC3_STEREO_MODE_AUTODETECT_DESCRIPTION = "自动检测";
        public const byte ATRAC3_STEREO_MODE_FORCE_JOINT_STEREO_VALUE = 0x01;
        public const string ATRAC3_STEREO_MODE_FORCE_JOINT_STEREO_DESCRIPTION = "联合立体声";
        public const byte ATRAC3_STEREO_MODE_FORCE_FULL_STEREO_VALUE = 0x02;
        public const string ATRAC3_STEREO_MODE_FORCE_FULL_STEREO_DESCRIPTION = "完全立体声";
        ...
        public const string XMA_STREAM_MODE_DEFAULT_DESCRIPTION = "默认";
        public const byte XMA_STREAM_MODE_SINGLE_VALUE = 0x01;
        public const string XMA_STREAM_MODE_SINGLE_DESCRIPTION = "单流";
        ...
        private void initializeTagHash()
        {
            ...
            tagHash.Add("GENH版本", enc.GetString(FileUtil.ReplaceNullByteWithSpace(this.versionNumber)));
            tagHash.Add("通道数", String.Format("0x{0}", BitConverter.ToUInt32(this.channels, 0).ToString("X4")));
            tagHash.Add("交错", String.Format("0x{0}", BitConverter.ToUInt32(this.interleave, 0).ToString("X4")));
            tagHash.Add("频率", String.Format("0x{0}", BitConverter.ToUInt32(this.frequency, 0).ToString("X4")));
            tagHash.Add("循环开始", String.Format("0x{0}", BitConverter.ToUInt32(this.loopStart, 0).ToString("X4")));
            tagHash.Add("循环结束", String.Format("0x{0}", BitConverter.ToUInt32(this.loopEnd, 0).ToString("X4")));
            tagHash.Add("标识符", String.Format("0x{0}", BitConverter.ToUInt32(this.identifier, 0).ToString("X4")));
            tagHash.Add("音频起始", String.Format("0x{0}", BitConverter.ToUInt32(this.audioStart, 0).ToString("X4")));
            tagHash.Add("GENH头部长度", String.Format("0x{0}", BitConverter.ToUInt32(this.headerLength, 0).ToString("X4")));
            tagHash.Add("原始文件名", enc.GetString(FileUtil.ReplaceNullByteWithSpace(this.originalFileName)));
            tagHash.Add("原始文件大小", String.Format("0x{0}", BitConverter.ToUInt32(this.originalFileSize, 0).ToString("X4")));
        }
        ...
        public void GetDatFileCrc32(ref Crc32 pChecksum)
        {
            ...
            using (FileStream fs = File.Open(this.filePath, FileMode.Open, FileAccess.Read))
            {
                using (BinaryReader br = new BinaryReader(fs))
                {
                    ...
                    {
                        pChecksum.Update(new ArraySegment<byte>(data, 0, bytesRead));
                    }
                }
            }
        }
