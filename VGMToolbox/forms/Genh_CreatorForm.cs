...
namespace VGMToolbox.forms
{
    public partial class Genh_CreatorForm : AVgmtForm
    {
        ...
        private GenhCreatorStruct getGenhParameters()
        {
            ...
            genhStruct.Format = drv != null ? Convert.ToUInt16(drv.Row.ItemArray[0]).ToString() : string.Empty;
            ...
            genhStruct.CoefficientType = drv != null ? Convert.ToByte(drv.Row.ItemArray[0]) : (byte)0;
            ...
            {
                object item = this.lbFiles.Items[i];
                if (item is ListBoxFileInfoObject listBoxFile)
                {
                    genhStruct.SourcePaths[j++] = listBoxFile.FilePath;
                }
                else
                {
                    System.Diagnostics.Trace.TraceError($"类型转换错误：期望ListBoxFileInfoObject，实际为{item.GetType()}");
                }
            }
            ...
            out string pErrorMessages)
        {
            ...
            if (!String.IsNullOrWhiteSpace(pGenhCreatorStruct.RawDataSize))
            {
                ...
                {
                    isValid = false;
                    errorBuffer.Append("原始数据大小必须大于零.");
                    errorBuffer.Append(Environment.NewLine);
                }
            }
        ...
        private bool isPs2AdpcmSelected()
        {
            ...
            DataRowView drv = (DataRowView)this.comboFormat.SelectedItem;
            UInt16 formatId = drv != null ? Convert.ToUInt16(drv.Row.ItemArray[0]) : (UInt16)0;
                ...
                try
                {
                    ...
                    if (loopValue >= 0)
                    {
                            ...
                            !String.IsNullOrWhiteSpace(this.cbChannels.Text))
                        {
                            ...
                            if (drv != null && GenhUtil.CanConvertBytesToSamples(formatId))
                            {
                            ...
                            {
                                MessageBox.Show("无法将字节转换为以下格式的样本:MPEG,XMA,XMA2,FFMPEG.", "Error");
                            }
                        }
                        else
                        {
                            MessageBox.Show("无法将字节转换为样本:请输入交错和通道的值.", "Error");
                        }
                    }
                }
                catch
