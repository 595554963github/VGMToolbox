...
num_samples = data_size";
        ...
        private void CreateTxthConfigFile(string pcmFilePath)
        {
            string txthFilePath = pcmFilePath + ".txth";

            if (!File.Exists(txthFilePath))
            {
                File.WriteAllText(txthFilePath, TXTH_CONFIG);
                ReportProgress(0, "已创建配置文件:" + Path.GetFileName(txthFilePath));
            }
        }
        ...
        protected override void DoTaskForFile(string pPath, IVgmtWorkerStruct pExtractSonyAdpcmStruct, DoWorkEventArgs e)
        {
            ...
            int fileCount = 0;
            string outputPath = Path.Combine(Path.GetDirectoryName(pPath), Path.GetFileNameWithoutExtension(pPath));
            ...
            using (FileStream fs = File.Open(pPath, FileMode.Open, FileAccess.Read))
            {
                ...
                {
                    outputFileName = String.Format("{0}_{1}.pcm", Path.GetFileNameWithoutExtension(pPath), fileCount.ToString("X8"));
                    ...
                    CreateTxthConfigFile(outputFilePath);
